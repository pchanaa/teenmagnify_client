import React, { Component } from 'react';
import "./local.css"
import "./modal.css"
import change from "./changeurl";
import Localinfo from "./localInfo";
import image from "./img/loading.png";
import {searchObject} from "./object";



export default class Local extends Component {

  state = {
    local : [],
    isLoading: true,
    openState : false,
    
  }
 
  splitReward=(str)=>{
    return str.split('금')[1];
  }
  splitInfo=(ele)=>{
    let array = ele.split('■').slice(1,4);
    
    console.log(array);
    return array;


  }
  changeStr=(data)=>{
    data = data.replace("신규","");
    data = data.replace("주최/주관","");
    data = data.replace("SPECIAL","");
    return data;
  }
  setDBdata=()=>{
    let str = window.location.pathname.split('/')[2]; 
    const {local}=this.state;
      if(str!==undefined){

            let array = local.filter(function(data){
              //console.log(data.part.slice(5).split(','))
              if(str==="chung" || str==="gyeong" || str==="jeolla"){
                return (data.local.includes(searchObject[str].slice(0,2)+'남'+searchObject[str].slice(2)) ||  data.local.includes(searchObject[str].slice(0,2)+'북'+searchObject[str].slice(2)))
              }
              else
                return data.local.includes(searchObject[str]);
        });
          /* return (array.map(data => 
            <Areainfo id={data._id} title={this.changeStr(data.title)} info={data.info} link={data.object} img={data.img} part={data.part.slice(5).split(',')} date={data.dday.split('D')[0].split('간')[1]} host={this.changeStr(data.host)}/>
          ));*/
            console.log(array);
            console.log(str);
          
          return (array.map(data => 
            <Localinfo key={data._id} local ={data.local} title={this.changeStr(data.title)} info={this.splitInfo(data.info)} link={data.object} img={data.img} part={data.part} date={data.dday.split('D')[0].split('간')[1]} host={this.changeStr(data.host)} first_reward={this.splitReward(data.price)} total_reward={this.splitReward(data.reward)}/>
          ));
      }
      else{
        return (local.map(data => 
          <Localinfo key={data._id} local ={data.local} title={this.changeStr(data.title)} info={this.splitInfo(data.info)} link={data.object} img={data.img} part={data.part} date={data.dday.split('D')[0].split('간')[1]} host={this.changeStr(data.host)} first_reward={this.splitReward(data.price)} total_reward={this.splitReward(data.reward)}/>
        ));
      }
    }
    
    
  
    
    
  
  getApi= ()=>{
    fetch('http://localhost:3001/api')
      .then(res => {
        //console.log(res.body)
        return res.json();
      }
        )
      .then(res => {
        let data = [];
        if(res.length%2 !== 0){
         for(let index = 0;index<res.length -1;index++)
          data[index] = res[index];
        }
        else{
          for(let index = 0;index<res.length;index++)
          data[index] = res[index];
        }
        //console.log(res.length)
        this.setState(element =>  ({local : data, isLoading:false}))})

      .catch(err=>console.log(err));
}

  componentDidMount(){
    const {isLoading} = this.state;
    //console.log(this.dbcondition)
    if(!isLoading){
      setTimeout(()=>{
        this.setState({isLoading: !isLoading});
      },6000);
    }
    this.getApi();
  }

  render() {
    const {isLoading} = this.state;
    return (
      <>
      {isLoading?
       (
        <div class="loading">
           <img alt="loading_img" src={image} className="loading_img"/>
        </div>
      ):
      <div className="local_box">
        <nav className="local_nav">
          <div className="local_nav_title">지역별 정보</div>
          <ul className="local_list">
          <li onClick={change.seoul}>서울특별시</li>
          <li onClick={change.incheon}>인천광역시</li>
          <li onClick={change.gyeonggi}>경기도</li>
          <li onClick={change.gangwon}>강원도</li>
          <li onClick={change.chung}>충청도</li>
          <li onClick={change.gyeong}>경상도</li>
          <li onClick={change.jeolla}>전라도</li>
          <li onClick={change.daejeon}>대전광역시</li>
          <li onClick={change.daegu}>대구광역시</li>
          <li onClick={change.gwangju}>광주광역시</li>
          <li onClick={change.ulsan}>울산광역시</li>
          <li onClick={change.busan}>부산광역시</li>
          <li onClick={change.jeju}>제주도</li>
          <li onClick={change.localetc}>기타</li>
         </ul>
         <button className="home_button" onClick={change.home}>홈으로</button>
         <section className="area_down_buttons">
            <img alt="facebook" src="data:image/png;base64,"/>
            <img alt="kakaotalk" src="data:image/png;base64,"></img>
            <img alt="instagram" src="https://workingwithdog.com/wp-content/uploads/2016/05/new_instagram_logo-1024x1024.jpg"></img>

         </section>
        </nav>
        <section className="local_container">
          <div className="local_div">
          <header className="local_c_header">
            <section className="local_header_box">
              <div className="local_header_high">우리 동네에서 이런 활동을 하는구나~~</div>
              <div className="local_header_down">우리 동네에서 진행되는 <br></br>다양한 활동들을 알아보자 </div>
            </section>
          </header>
          <section className="local_items">
            {this.setDBdata()}
            </section> 
          </div>
        </section>
      </div>
  }
      </>
    )
  }
}
